...
api_patterns = [
    ...
    # WRAPPER: safeFetchJson with optional <T>
    (re.compile(r'\bsafeFetchJson(?:<[^>]+>)?\s*\('), "Özel API fetch wrapper çağrısı (safeFetchJson)"),
            ...
            for i, line in enumerate(lines):
                ...
                for pattern, desc in api_patterns:
                    if pattern.search(line_str):
                        ...
                        if 'safeFetchJson' in pattern.pattern:
                ...
                if not matched and ('Math.sin(' in line_str or 'Math.random()' in line_str):
                    l_lower = line_str.lower()
                    if 'price' in l_lower or 'data' in l_lower or 'basep' in l_lower or 'target' in l_lower:
                        api_findings.append({
                                'file': filepath,
                                'line': i + 1,
                                'code': line_str,
                                'desc': "Sahte/Simüle Veri Üretimi (Matematiksel)"
                        })
                        has_api = True

                if not matched and 'endpointSample' in line_str and 'id:' not in line_str:
                     l_lower = line_str.lower()
...
print(f"Total safeFetchJson files: {len(wrapper_usages)}")
print(f"Total non API files: {len(non_api_files)}")

# Output WRAPPER_FONKSIYONLARI.md
with open("WRAPPER_FONKSIYONLARI.md", "w", encoding="utf-8") as f:
    f.write("# Projedeki API Wrapper Fonksiyonları\n\n")
    f.write("## `safeFetchJson` (Özel Fetch Yöneticisi)\n")
    f.write(f"- **Tanımlandığı Yer:** `src/utils/apiClient.ts`\n")
    f.write(f"- **Toplam Çağrıldığı Dosya Sayısı:** {len(wrapper_usages)}\n")
    f.write("- **Dosya Listesi:**\n")
    for fp in sorted(wrapper_usages.keys()):
        f.write(f"  - `{fp}` ({wrapper_usages[fp]} kez kullanılmış)\n")

# Output final markdown files
with open("API_KAYNAKLARI_NIHAI_DUZELTILMIS.md", "w", encoding="utf-8") as f:
    f.write("# API ve Veri Kaynağı Bulunan Dosyalar (Düzeltilmiş)\n\n")
    ...
    for item in api_findings:
        ...
        if len(code) > 120:
            code = code[:117] + "..."
        f.write(f"| `{item['file']}` | {item['line']} | `{code}` | {item['desc']} | Konsolidasyon hatası sonrası düzeltildi |\n")

with open("API_BAGLANTISI_OLMAYAN_DOSYALAR_NIHAI_DUZELTILMIS.md", "w", encoding="utf-8") as f:
    f.write("# API Bağlantısı Olmayan Dosyalar (Düzeltilmiş)\n\n")
    non_api_files.sort()
