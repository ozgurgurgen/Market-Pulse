...
const PARAMS: any = {
  BIST:     { entry: 80, atrMult: 1.5, ts: 14, rr: 2.5, cost: 0.0030, posSize: 0.07 },
  US_STOCK: { entry: 75, atrMult: 2.0, ts: 10, rr: 2.5, cost: 0.0010, posSize: 0.07 },
  US_ETF:   { entry: 80, atrMult: 2.0, ts: 14, rr: 2.5, cost: 0.0010, posSize: 0.07 }
};
...
// ----------------------------------------------------
// G. TAM KAYNAK KOD ZORUNLULUĞU (8 ÇEKİRDEK FONKSİYON)
// ----------------------------------------------------
...
function generateCompositeScore(out: any[], i: number) {
    ...
    // RASTGELE BİLEŞEN TAMAMEN KALDIRILDI!
...
function updateTrailingStop(pos: any, bar: any): number {
    // İzsüren Stop: Sadece yukarı yönlü hareket eder
...
async function runPortfolioBacktest() {
    console.log("Fetching Data for v6.4...");
    ...
    let duplicatePosError = false;
    
    for (const date of dates) {
        let nextOpen = [];
        
        // Hata Kontrolü: Aynı varlığa birden fazla pozisyon var mı?
        let tickerCounts: Record<string, number> = {};
        for (const p of openPositions) {
            tickerCounts[p.asset] = (tickerCounts[p.asset] || 0) + 1;
            if (tickerCounts[p.asset] > 1) duplicatePosError = true;
        }
        ...
        for (const pos of openPositions) {
            ...
            // TRAILING STOP GÜNCELLEMESİ (Her gün yukarı taşır)
            pos.sl = updateTrailingStop(pos, bar);
            
            if (bar.low <= pos.sl) { exitPrice = pos.sl; reason = 'TRAILING_STOP'; isClosed = true; }
            ...
            if (isClosed) {
                ...
                if (netPnL < -0.05) { 
                    largeLosses.push({
                        ...
                        rca: 'Sınırlı Stop (%5) patladı veya piyasa boşluklu (gap) açıldı.'
                    });
                }
            } else {
                nextOpen.push(pos);
        ...
        for (const asset of ASSET_LIST) {
            ...
            // KRİTİK DÜZELTME: Aynı varlıkta açık pozisyon varsa yeni sinyali REDDET!
        ...
        for (const sig of signals) {
            ...
            // Maksimum Stop Mesafesi: ATR Çarpanı ile Giriş Fiyatının %5\'i arasından KÜÇÜK olanı al
            let rawSlDist = sig.bar.atr14 * prm.atrMult;
            let maxSlDist = sig.bar.close * 0.05; 
            let slDist = Math.min(rawSlDist, maxSlDist);
            
            let riskPct = slDist / sig.bar.close;
            let totalRisk = openPositions.reduce((acc, p) => acc + (p.riskPct * p.posSize), 0);
            
            if (totalRisk + (riskPct * prm.posSize) > 0.12) continue;
            ...
            openPositions.push({
                ...
                riskPct, posSize: prm.posSize, holdingDays: 0, slDist: slDist
            });
            executedToday++;
        }
    }
    ...
    let first20 = tradeLog.slice(0, 20);
    let last20 = tradeLog.slice(Math.max(0, tLen - 20), tLen);
...
* Aynı varlığa mükerrer (duplicate) pozisyon açılması kesin olarak engellenmiştir. **(Duplicate Kontrolü: ${duplicatePosError ? '🔴 BAŞARISIZ' : '🟢 GEÇTİ'})**
...
|---|---|---|---|
| **İşlem Sayısı** | ${metrics.n} | 300 - 600 | ${metrics.n >= 300 && metrics.n <= 600 ? '🟢 PASS' : '🔴 FAIL'} |
...
\`\`\`
`;
    fs.writeFileSync('server/backtest/reports/v6.4-final-report.md', report);
    console.log("v6.4 Bitti! Rapor kaydedildi.");
}
