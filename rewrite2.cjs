...
function replaceBlock(routePrefix, newCode) {
    ...
    if (endIdx === -1) {
        const fallbackEndIdx = code.indexOf(`});\n// ====`, startIdx);
        if (fallbackEndIdx === -1) {
            console.error("END NOT FOUND", routePrefix);
            return;
        }
        code = code.substring(0, startIdx) + newCode + code.substring(fallbackEndIdx + 4);
    } else {
        code = code.substring(0, startIdx) + newCode + code.substring(endIdx + 4);
    }
}
